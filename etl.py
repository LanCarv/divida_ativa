...
def execute_sql_query(query, output_path):
...
def upload_to_s3(csv_buffer, s3_bucket, s3_key):
    s3 = boto3.resource(
        's3',
...
if __name__ == "__main__":
    ...
    csv_buffer_1 = execute_sql_query(query_1, 'imovel_mercantil.csv')

    print("Iniciando Segunda consulta da base de notas")
    # Consulta 2
    query_2 = """select * from public.vw_py_nfse_2anos"""
    csv_buffer_2 = execute_sql_query(query_2, 'emissao_notas.csv')
    ...
    with io.BytesIO() as zip_buffer:
        ...
        zip_buffer.seek(0)
        upload_to_s3(zip_buffer, os.getenv("S3_BUCKET_NAME"), s3_key_zip)
