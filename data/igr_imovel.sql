SELECT
concat('imovel-', fda.termo_da_unico,fda.séerie_da_cda) as CDA,
'imovel' as tipo_divida,
count(termo_da_unico) as DEB_TOTAIS,

sum(CASE WHEN fda.chave_dim_situacao_processo in (2, 103,105)
...
sum(CASE WHEN fda.chave_dim_tributo not in (5, 8, 19) THEN fda.valor_total
   else 0 end) as VLR_TAXA,

sum(CASE WHEN fda.chave_dim_situacao_processo in (2, 103,105)   THEN fda.valor_total
    ELSE 0 END) AS VALOR_PAGO,

max(fda.chave_tempo_inscricao) AS inscricao_divida,
max(dm.chave_dim_imovel) AS id_pessoa,

max(dm.situacao_imovel) AS SITUACAO,

max(CASE WHEN dm.natureza_imovel = 'T'  THEN 0  ELSE 1 END) AS EDIFICACAO,
...
FROM
    ...
    LEFT JOIN dim_imovel_aquila_uni dm on fda.chave_dim_imovel = dm.chave_dim_imovel
